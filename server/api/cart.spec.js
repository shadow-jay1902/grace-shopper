...
describe.only('Order routes', () => {
    ...
    describe('Add items to cart', () => {
        ...
        it('Should have all of the items that were added to the users cart', async () => {
            ...
            const { body } = await agent.post('/api/cart')
                ...
                .expect(201)
            Utils.expectCart(body, 'user', user.id)
            expect(body.items).to.deep.equal([{ ...items[0], id: 1, quantity: 2 }, { ...items[1], id: 2, quantity: 4 }])
        })
    ...
    describe('Update cart', () => {
        ...
        it('Should update the quatity of items in a users cart', async () => {
            ...
            Utils.expectCart(body, 'user', user.id)
            expect(body.items).to.deep.equal([{ ...items[0], id: 1, quantity: 5 }, { ...items[1], id: 2, quantity: 4 }])
        })
